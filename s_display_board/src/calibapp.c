...
paintMessage(SWIM_WINDOW_T *win, char* pMsg)
{
  ...
  swim_put_box(win, 0, 0, win->xpsize, 40);
  swim_set_pen_color(win, WHITE);
  
  // each character is 6 pixels wide
  x = (win->xpsize / 2) - (msgLen*6/2);
...
static void
updatePointsForWindow(SWIM_WINDOW_T *win)
{
  refPoint1.x = 15; 
  refPoint1.y = win->ypsize - 15;
  refPoint2.x = win->xpsize / 2; 
  refPoint2.y = 80;
  refPoint3.x = win->xpsize - 15; 
  refPoint3.y = win->ypsize - 15;
}
